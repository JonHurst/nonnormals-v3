...
\section{Wing tank pump(s) low pressure}
...
\section{Center tank pumps low pressure}
...
\section{Outer tank transfer valves failed closed}

If both transfer valves fail to open when a wing tank reaches low level, the
fuel in that outer tank becomes unusable. The fuel balance will remain within
limits since maximum outer tank imbalances are acceptable if the total fuel in
either wing is the same (see \ac{FCOM}~\cphrase{LIM.FUEL}).

\multicite{\uline{FUEL}~L(R)~XFR~VALVE~CLOSED, FCOM~PRO.AEP.FUEL}

\section{Outer tank transfer valve open out of sequence}

Maximum outer tank imbalances are acceptable if the total fuel in either wing is
the same (see \ac{FCOM}~\cphrase{LIM.FUEL}], so no action is required.

\multicite{\uline{FUEL}~L(R)~XFR~VALVE~OPEN, FCOM~PRO.AEP.FUEL}
