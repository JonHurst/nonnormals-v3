...
\ifdefined\HCode
\else
%% Widens the section number in contents list to account for
%% long section numbers such as 10.10
\makeatletter
\def\l@section{\@dottedtocline{1}{1em}{2.75em}}
\makeatother
\tableofcontents
\fi
...
with a target pitch attitude of 11\textdegree{ }and minimal vertical speed,
landing parallel to the swell unless there are strong crosswinds, in which case
an into wind landing is preferred. Forced landing will be gear down with the
spoilers armed.

The aircraft should be depressurised for the landing, with the
\cphrase{Ditching} \ac{PB} pushed for the ditching case. For forced landings or
for ``without power'' ditching this is achieved using \ac{RAM AIR}. For ``with
power'' ditching, depressurise by turning off all bleeds, which provides a
slightly more watertight hull.
...
\section{IDG low oil pressure/high temperature}
\label{sec-idg}
...
suitable level and below 200kt, as a last resort \ac{PM}'s cockpit window can
be opened.
...
will necessitate \ac{PF} donning a crew oxygen mask and \ac{PM} donning the
smoke hood.
...
have closed, the gear will rest on the doors so avoid excessive g loads. If the doors have not
...
In general, the rudder should be used with caution when above 160kt.

An attempt should be made to recover the limiter by resetting the \ac{FAC}s.  If
unsuccessful, full rudder travel authority may or may not be recovered at slat
extension. If a landing must be made with the rudder travel limiter frozen in
the high speed regime, max crosswind is reduced to 15kt and differential braking
may be required on the landing roll (do not arm autobrake).
...
\section{Double AOA heat fail}

If two \ac{AOA} probes are affected by icing, the computers may erroneously
deselect the remaining good \ac{ADR}. Switching off one of the affected
\ac{ADR}s leaves the system in the state described in Section
\ref{sec-adr-disagree}.

\multicite{QRH~AEP.A-ICE, FCOM~PRO.AEP.A-ICE}


\section{Multiple pitot heat failures}

The issue with loss of anti-ice on more than one pitot probe is that it is
possible that the two unprotected pitot probes will ice up at the same time and
provide erroneous but coherent data. This leads to a situation where the
\ac{ADR} associated with the remaining protected probe is locked out despite
being the single correct source.

Obviously, the first thing to do is to avoid icing conditions.

If there is a working \ac{ADR} connected to a protected probe, turn one of the
\ac{ADR}s associated with an unprotected probe off. This ensures that an
\cphrase{ADR DISAGREE} \ac{ECAM} caution is triggered by significant speed
discrepancies and ensures that the protected \ac{ADR} will not be automatically
deselected.

If pitot heat is lost on all probes, one of the \ac{ADR}s should, again, be
turned off to ensure the \cphrase{ADR DISAGREE} \ac{ECAM} caution is provided. If
icing is expected, turn off a second \ac{ADR} and be ready to apply unreliable
airspeed procedures (see Section \ref{sec-unreliable-airspeed}).

\multicite{\uline{ANTI~ICE}~ALL(CAPT(F/O)+F/O(STBY))~PITOT, FCOM~PRO.AEP.A-ICE}


\section{Single pitot probe heat or static port heat fault}

The \ac{ADR} associated with the failed probe or port should be considered
unreliable. \ac{ADR}1 or \ac{ADR}2 can be replaced with \ac{ADR}3 using air data
switching. If using standby instruments with \ac{ADR}3 unreliable, air data
information must be monitored closely.

\multicite{\uline{ANTI~ICE}~CAPT(F/O)(STBY)~PITOT(L(R)~STAT),
  FCOM~PRO.AEP.A-ICE}


\section{Single AOA or TAT heat fault}

No immediate operational effect.

\multicite{\uline{ANTI~ICE}~CAPT(F/O)(STBY)~AOA(TAT), FCOM~PRO.AEP.A-ICE}


\section{Probe heat computer failure}

If applicable, deselect the affected ADR.

\multicite{\uline{ANTI~ICE}~CAPT(F/O)(STBY)~PROBES, FCOM~PRO.AEP.A-ICE}


\section{Window heat fault}

No immediate operational effect.

\multicite{\uline{ANTI~ICE}~L(R)(L+R)~WINDSHIELD(WINDOW), FCOM~PRO.AEP.A-ICE}


\section{Engine anti-ice valve fault}

If a valve fails to open when commanded, avoid icing conditions. If it fails to
close when commanded, a thrust limit penalty applies.
% TODO: Check this is automatically applied by the relevant FADEC.

\multicite{\uline{ANTI~ICE}~ENG~1(2)~VALVE~OPEN(CLSD), FCOM~PRO.AEP.A-ICE}


\section{Wing anti-ice valve faults}
%\section{Wing anti-ice valve open when commanded closed}

If a wing anti-ice valve is open when commanded closed:

\begin{itemize}
\item In the air, just allow the failed side to be continually anti-iced and use
  wing anti-ice on the working side when required. A thrust limit penalty will
  apply.
\item On the ground, if this occurs after the ground self-test, simply switch
  off the wing anti-ice with the \ac{PB}. Otherwise, isolate and depressurise
  the pneumatic system on the failed side.
\end{itemize}

%\section{Wing anti-ice valve closed when commanded open}

If a wing anti-ice valve is closed when commanded open, the wing anti-ice must
be turned off to avoid asymmetrically de-icing the wings. Avoid icing
conditions. If ice accretion does occur, landing distances and \V{APP}
adjustments are in \ac{QRH} and speed must be maintained above the higher of
\V{LS} + 10kt or Green Dot.

\multicite{\uline{WING~ANTI~ICE}~SYS~FAULT, FCOM~PRO.AEP.A-ICE,
  FCOM~PRO.NOR.SUP.AW}


\chapter{Indicating/Recording}

\section{Display unit failure}

Intermittent flashing of \ac{DU}s may be indicative of a generator issue. If
\ac{cm}1 \ac{DU}s are flashing, try turning off generator 1. If \ac{cm}2
\ac{DU}s are flashing, try turning off generator 2. The \ac{APU} generator can
be used if successful.

In the case of a blank \ac{DU}, a large amber ``F'', a distorted display or
minimum brightness, on some airframes a reset may be attempted by selecting the
\ac{DU} brightness off then back on. If the reset is not applicable or the
\ac{DU} does not recover after 10 seconds, the affected \ac{DU} can be turned
off. In the case of an \cphrase{INVALID DISPLAY UNIT} message, an automatic
recovery attempt is initiated; this can take 40 seconds or more. If automatic
recovery does not succeed, the \ac{DU} can be turned off.

An \cphrase{INVALID DATA} message is indicative of a \ac{DMC} problem; \ac{EIS}
\ac{DMC} switching may recover the \ac{DU}, as may turning the \ac{DU} off then
on. It is possible that this message will appear simultaneously on \emph{all}
\ac{DU}s; this initiates an automatic recovery attempt, which, again, can take
40 seconds or more. \ac{DU}s that are not automatically recovered may be
recovered manually by sequentially turning them off for 40 seconds then back
on. If resetting a \ac{DU} triggers a recurrence of the original problem, leave
that \ac{DU} off for the next recovery cycle.

In the event of an unrecoverable failure of a display unit, some automatic
display switching will occur: \ac{PFD} will auto-transfer to \ac{NDU}s and
\ac{EWD} will auto-transfer to \ac{SDU}. Each pilot has a \cphrase{PFD/ND XFR}
button that allows their \ac{PFD} to be toggled onto their \ac{NDU} and their
\ac{ND} to be toggled onto their \ac{PFDU}. An \cphrase{ECAM/ND XFR} switch on
the switching panel allows either the \ac{SD} (when \ac{SDU} \strong{xor}
\ac{EWDU} failed) or \ac{EWD} (\ac{SDU} \strong{and} \ac{EWDU} failed) to be
displayed on an \ac{NDU}.

The most likely end result of a \ac{DU} failure is that one \ac{ND} must be
shared and/or that the \ac{SD} is not displayed. The \ac{ECAM} can be operated
with just the \ac{EWD}. When a \ac{SYS} page is needed, press and hold the
required \ac{SYS} page button. When \cphrase{STATUS} is required, press and
maintain the \ac{STS} key. The \ac{EWD} will be displayed 2s after the \ac{STS}
key is released. To access \cphrase{STATUS} overflow, release then repress the
\ac{STS} key within 2s.

\multicite{QRH~AEP.EIS, FCOM~PRO.AEP.EIS}


\section{Multiple spurious ECAM alerts}

A faulty \ac{DMC} can cause multiple spurious \ac{ECAM} alerts (spurious as
confirmed by \ac{SD} pages). The \ac{EIS} \ac{DMC} switch should be used to
identify the faulty \ac{DMC} and replace it with \ac{DMC}3.

\multicite{QRH~AEP.FWS, FCOM~PRO.AEP.FWS}


\section{Flight Warning Computer failure}

The two identical \ac{FWC}s generate alert messages, memos, aural alerts, and
synthetic voice messages. Loss of a single \ac{FWC} leads to downgrade to Cat 3
Single due to loss of redundancy. The major effects of loss of both \ac{FWC}s
are loss of \ac{ECAM} (including attention getters, status page and
takeoff/landing memos), loss of auto callouts\footnote{There is some question as
to whether windshear and \ac{GPWS} aural alerts are included under `auto
callouts'. The \ac{FCOM} is unclear on the matter.} (radio height and decision
height) and loss of altitude alerting. In the sim, there is also loss of rudder
trim reset, loss of \ac{APU} fuel valve status and loss of cabin crew call alert
as side-effects.

The procedure is simply to use the \ac{SYS} pages and overhead panel warning
lights to monitor the systems, and for \ac{PM} to make the relevant
callouts. Alternative method for cabin crew to get flight deck attention may be
required.

\multicite{\uline{FWS}~FWC~1(2)(1+2)~FAULT, FCOM~PRO.AEP.FWS}


\chapter{Pneumatic}

\section{Dual bleed failure}

The historical failure case where supplying two packs from one bleed would
overload it and cause it to fail appears to have been resolved. Furthermore,
recent airframes have four new \ac{ECAM} procedures associated with dual bleed
failure, the variants based on whether neither, either or both bleeds have
failed due to a wing or pylon leak on its respective side. The \ac{ECAM}s all
hand off to a new \ac{QRH} paper procedure if no successful reset is
achieved. Older airframes make do with just the \ac{QRH} procedure.

Whilst the procedures look complex, the underlying philosophy is fairly simple:

\begin{itemize}
\item A depressurisation will be underway. It should be relatively slow, but
  unless a successful reset is achieved immediately, a prompt descent to
  \ac{FL}100/\ac{MFA} needs to be initiated.
\item A bleed lost to a wing or pylon leak is lost for good, whereas a simple
  bleed fault or bleed low pressure may be recoverable. The cross bleed should
  be shut and the wing anti-ice should be off before an attempt is made to
  recover a bleed.
\item A leak on the left side precludes the use of the \ac{APU} bleed for
  pressurisation, but otherwise the \ac{APU} bleed can be used to supply pack 1
  when below 22500ft. If pack 1 is unavailable, the \ac{APU} bleed can supply
  pack 2 via the cross bleed provided the whole pneumatic system is available.
\end{itemize}

In line with this philosophy, both the \ac{ECAM} and \ac{QRH} procedures start
by shutting the cross bleed and turning off the wing anti-ice. The \ac{ECAM}
procedures each then attempt an immediate recovery of any recoverable bleed by
cycling the bleed off then on. The \ac{QRH} attempts an immediate recovery only
in the case that both bleeds are potentially recoverable.

If no bleeds are recovered, the bleeds are all turned off and a descent to
\ac{FL}100/\ac{MFA} is initiated. At this point, the \ac{ECAM} procedures join
the \ac{QRH} procedure.

During the descent the \ac{APU} is started. If the left hand side of the
pneumatic system is available, at attempt to use the \ac{APU} bleed for
pressurisation is made at \ac{FL}200. If this is successful, the descent is
stopped at \ac{FL}200 and a further attempt is made to reset the engine bleeds.

If \ac{APU} cannot be used for pressurisation descent must be continued. When
level at \ac{FL}100/\ac{MFA}, a final attempt is made to restore any available
engine bleeds. If unsuccessful, the flight is completed with ram air.

\multicite{\uline{AIR} 1+2 BLEED FAULT (LEFT(RIGHT) LEAK (AND RIGHT LEAK)),\\
  QRH~AEP.AIR, FCOM~PRO.AEP.AIR}


\chapter{Communications}

\section{Communication failure (ICAO)}

If all attempts to establish contact fail, transmit message twice on designated channel,
proceeded by the phrase ``Transmitting Blind.''

Set Mode A Transponder Code to 7600.

If in \ac{VMC}, continue in \ac{VMC} to the nearest suitable airfield and land,
reporting arrival to appropriate \ac{ATS} by the most expeditious means.

If in \ac{IMC} or unlikely to be able to maintain \ac{VMC}:

\begin{itemize}
\item If in airspace where a \strong{procedural} separation service is being
  applied, maintain speed and level, or minimum flight altitude if higher, for a
  period of \strong{20 minutes} following failure to report over a compulsory
  reporting point, then adjust level and speed in accordance with flight plan.
\item If in airspace where an \ac{ATS} surveillance system is in use, maintain
  speed and level, or minimum flight altitude if higher, for \strong{7 minutes}
  after Code 7600 is set, then adjust level and speed in accordance with flight
  plan.
\item If being radar vectored or having been directed to proceed offset using
  \ac{RNAV} without specified limit, rejoin the flight plan route no later than
  the next significant point.
\item On reaching the appropriate \ac{NAVAID} or fix serving the destination
  aerodrome, hold if necessary, then at \ac{EAT}, or if no \ac{EAT} has been
  received and acknowledged, computed flight plan \ac{ETA}, commence an
  appropriate instrument approach procedure. If possible land within 30 minutes
  of this time.
\item Watch for visual signals from the tower:
  \begin{itemize}
  \item Red Pyro: Permission to land cancelled
  \item Flashing Red Light: Land elsewhere
  \item Continuous Red: Give way to other aircraft, continue circling
  \item Flashing Green: Return to circuit, await landing clearance
  \item Continuous Green: Permission to land
  \end{itemize}
\end{itemize}


\section{Communication failure (UK airspace)}

The UK procedures expand on the \ac{ICAO} procedures:

\begin{itemize}
\item If following a \ac{SID} fly the published lateral and vertical profiles,
  including any step climb, until the last waypoint of the procedure is
  reached. Maintain current speed and last assigned level (or minimum flight
  altitude if this is higher) until \strong{7 minutes} have elapsed since setting
  7600. Then adjust speed and level in accordance with current flight plan. This
  procedure also applies to \ac{RNP}-1 (\ac{PRNAV}) departures.
\item If following a \ac{STAR}, follow the lateral profile but maintain current
  speed and last assigned level (or minimum flight altitude if this is higher)
  until \strong{7 minutes} have elapsed since setting 7600. Subsequently, arrange
  descent to be as close as possible to published planning profile. If no
  profile is published, arrange to be at the \ac{IAF} at minimum published
  level. This procedure also applies to \ac{RNP}-1 (\ac{PRNAV}) arrivals.
\item If under radar vectors from Approach Control Radar, comply with
  instructions on radar vectoring chart. If under radar vectors without
  specified limit from other \ac{ATS} unit, continue in accordance with last
  instructions until \strong{3 minutes} have elapsed since setting 7600, then
  proceed in most direct manner to rejoin current flight plan route. If
  necessary, climb to minimum flight altitude.
\item If performing an \ac{SRA}, continue visually or by using an alternate
  approach aid. If this is not possible, carry out a missed approach and
  continue to the holding position of a suitable aerodrome with a notified
  instrument approach and carry out that procedure.
\end{itemize}

\multicite{ICAO~Annex~10, ICAO~DOC~4444, UKAIP~ENR~1.1.3.4}


\chapter{Miscellaneous Tips}

\begin{itemize}
\item When configuring, \V{LS} is more critical than \V{FE}, which in turn is
  more critical than manoeuvring speeds (S and F speeds). Both \V{LS} and \V{FE}
  are trustworthy with jammed flaps/slats.
\item When configuring on approach, flaps do not begin to extend until \ac{CONF} 2 is
selected.
\item Slat position does not change between \ac{CONF} 2 and \ac{CONF} 3.
\item Dual hydraulic failure fundamentals:
  \begin{itemize}
  \item You will \emph{always} need to gravity extend the gear, so you will
    never be able to retract it on the go-around.
  \item If you only have blue, you lose the flaps; if you only have yellow, you
    lose the slats. The flaps/slats jammed procedure (including go-around
    modifications) is therefore incorporated into both these procedures.
  \item If you only have blue or yellow, it will be a Direct Law landing; Normal
    Law is retained if green is available. Direct law is triggered by gear
    extension. If you only have blue, the \ac{THS} is frozen, so you need to
    trigger Direct Law \emph{late} (at \ac{Conf} 3, \V{APP}) to get neutral trimming of
    the elevators. If you only have yellow, the aircraft handling is sluggish;
    trigger Direct Law \emph{early} (at 200kt) to improve responsiveness.
  \item You're going to need a longish runway. Worst case is if you only have
    blue, since you have, at best, accumulator braking and you're coming in fast
    due to lack of flaps. Yellow is better since you have alternate braking, and
    its mainly about the lack of slats. If you have green, its not really all
    that bad.
  \end{itemize}
\end{itemize}
