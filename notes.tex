...
\ecam{misc}{One Engine Inoperative -- Circling Approach}. If affected, plan a
\ac{FLAP} 3 landing and delay gear extension until level flight is no longer
required; expect a \cphrase{L/G NOT DOWN} warning below 750ft (can be silenced
with the \ac{EMER CANC} pb) and a \cphrase{TOO LOW GEAR} below 500ft \ac{RA}.
...
possible to be fully stalled in the short term unreliable airspeed
...
If engine \ac{EGT} limits are exceeded, consider precautionary engine shutdown
...
on. Initiate a descent; if above \ac{FL}160, this should be according to <xref
linkend="emerg-descent"/>. Once the descent is established and all relevant
checklists are complete, check the position of the outflow valve and, if it is
not fully closed, use manual control to close it.
...
The PACK FAULT \ac{ECAM} indicates that the pack flow control valve position
disagrees with the selected position or that the pack valve has closed due to
compressor outlet overheat or pack outlet overheat. The affected pack should be
turned off. A possible reason for this failure is loss of both channels of an
Air Conditioning System Controller (\ac{ACSC}). If this occurs, the associated hot
...
by cycling the \ac{HOT} \ac{AIR} button. If recovery is not possible, basic temperature
regulation will continue to be provided by the packs.
...
effects will depend on its actual position. If it is closed when commanded open,
the packs will provide basic temperature regulation. More serious is if it has
been commanded closed in response to a duct overheat and it fails to
close. Manual control may be effective, but if it is not the only option is to
turn off both packs and proceed as per <xref linkend="pack-fault"/>.
...
automatically close on application of take-off power. The \ac{ECAM} Cab Press page
will differentiate.
...
try again after splitting the systems with the \ac{BUS} \ac{TIE} button.
...
So much equipment is lost in the emergency configuration that QRH~AEP.ELEC
provides a table of surviving equipment. Notable losses are:

\begin{itemize}
\item All the fuel pumps, requiring Gravity Fuel Feeding procedures to be used
  (see <xref linkend="gravity-feeding"/>) and making center tank fuel
  unusable.

\item The anti-skid, three fifths of the spoilers and the reversers. Combined
  with the higher landing speeds required to prevent \ac{RAT} stall this results in
  significantly increased landing distances.

\item Alternate Law with reduced protections. Mechanical yaw becoming Alternate
  Law yaw with \ac{FAC}1 reset. Anticipate Direct Law at gear extension and be
  prepared for a Direct Law go around. Manual pitch trim is
  available.

\item Anti-icing for probes supplying the standby instruments: in case of
  disagreement, go with the \ac{CM}1 instruments.

\item Nose Wheel Steering. Use differential braking to clear the
  runway.
\end{itemize}

QRH~AEP.ELEC also provides a paper summary which should be applied once \ac{ECAM}
