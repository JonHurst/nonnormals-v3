...
Obviously, the first thing to do is to avoid icing conditions. If there is a
working \ac{ADR} connected to a protected probe, turn one of the \ac{ADR}s
associated with an unprotected probe off. This ensures that an \cphrase{ADR
  DISAGREE} \ac{ECAM} caution is triggered by significant speed discrepancies
and ensures that the protected \ac{ADR} will not be automatically deselected.
...
if unsuccessful, the flight can be continued at \ac{FL}200 using the \ac{APU}
bleed for pressurisation.
